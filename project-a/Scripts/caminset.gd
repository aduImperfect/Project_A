...
func _process(_delta: float) -> void:
	...
	print(insetSize)
	get_child(0).scale = insetSize

	# LEFT INSET
	if (playerPos.x <= (global_position.x - (insetSize.x / 2))):
		get_child(0).get_child(0).texture = highlightedTex
		global_position.x -= _delta * InputsData.max_move_speed
		CameraHelper.position_cam.x -= _delta * InputsData.max_move_speed
	...
	# TOP INSET
	if (playerPos.y <= (global_position.y - (insetSize.y / 2))):
		get_child(0).get_child(2).texture = highlightedTex
		global_position.y -= _delta * InputsData.max_jump_speed
		CameraHelper.position_cam.y -= _delta * InputsData.max_jump_speed
